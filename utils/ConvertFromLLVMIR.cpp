...
// Note: This file is the extension of same file in llvm-project with addtional instructions support.
//===----------------------------------------------------------------------===//
...
private:
  ...
  Value processInlineAsm(llvm::InlineAsm*);
...
Location Importer::processDebugLoc(const llvm::DebugLoc &loc,
  ...
  return FileLineColLoc::get(context, "imported-bitcode", loc.getLine(), loc.getCol());
}
...
Value Importer::processConstant(llvm::Constant *c) {
  ...
  if (Attribute attr = getConstantAsAttr(c)) {
    ...
    if (auto symbolRef = attr.dyn_cast<FlatSymbolRefAttr>()) {
      val = bEntry.create<AddressOfOp>(unknownLoc, type, symbolRef.getValue());
      return instMap[c] = val;
    }
    val = bEntry.create<ConstantOp>(unknownLoc, type, attr);
      return instMap[c] = val;
  }
  if (auto *cn = dyn_cast<llvm::ConstantPointerNull>(c)) {
    ...
    val = bEntry.create<NullOp>(unknownLoc, type);
      return instMap[c] = val;
  }
  if (auto *GV = dyn_cast<llvm::GlobalVariable>(c)) {
    Value val = bEntry.create<AddressOfOp>(UnknownLoc::get(context), processGlobal(GV));
    return val;
  }
  ...
  if (auto *ue = dyn_cast<llvm::UndefValue>(c)) {
    ...
    val = bEntry.create<UndefOp>(UnknownLoc::get(context), type);
    return instMap[c] = val;
  }

  if (auto* k = dyn_cast<llvm::ConstantAggregateZero>(c)) {
    Type type = processType(k->getType());
    bool is_vector = llvm::VectorType::classof(k->getType());
    Value undef = bEntry.create<UndefOp>(UnknownLoc::get(context), type);

    DenseMap<llvm::Value *, Value> constMap;
    IntegerAttr index;
    Value position;
    Value val;

    for (unsigned i = 0; i < k->getElementCount().getValue(); ++i) {
      if (is_vector) {
        mlir::Attribute attr = b.getIntegerAttr(b.getI64Type(), llvm::APInt(64, i, true));
        OpBuilder builder(currentEntryBlock, currentEntryBlock->begin());
        position = builder.create<ConstantOp>(UnknownLoc::get(context), b.getI64Type(), attr);
      } else {
        index= b.getI64IntegerAttr(i);
      }

      llvm::Constant* v = k->getElementValue(i);
      auto it = constMap.find(v);
      if (it != constMap.end()) {
        val = it->second;
      } else {
        val = processConstant(v);
        constMap[v] = val;
      }

      if (is_vector) {
        undef = bEntry.create<InsertElementOp>(UnknownLoc::get(context), type, undef, val, position);
      } else {
        undef = bEntry.create<InsertValueOp>(UnknownLoc::get(context), undef, val, b.getArrayAttr(index));
      }
    }

    return instMap[c] = undef;
  }

  if (auto* k = dyn_cast<llvm::ConstantAggregate>(c)) {
    Type type = processType(k->getType());
    bool is_vector = llvm::VectorType::classof(k->getType());
    Value undef = bEntry.create<UndefOp>(UnknownLoc::get(context), type);
    ...
    for (unsigned i = 0; i < k->getNumOperands(); ++i) {
      if (is_vector) {
        position = b.create<ConstantOp>(UnknownLoc::get(context), b.getIndexType(), b.getIndexAttr(i));
      } else {
        index= b.getI64IntegerAttr(i);
      }

      llvm::Constant* v = k->getOperand(i);
      auto it = constMap.find(v);
      if (it != constMap.end()) {
        val = it->second;
      } else {
...
Value Importer::processInlineAsm(llvm::InlineAsm* i) {
  Type type = processType(i->getType());
  std::string asm_string = i->getAsmString();
  std::string constraints = i->getConstraintString();
  bool has_side_effects = i->hasSideEffects();
  bool is_align_stack = i->isAlignStack();
  mlir::LLVM::AsmDialectAttr asm_dialect = mlir::LLVM::AsmDialectAttr::get(context, (mlir::LLVM::AsmDialect)i->getDialect());

  OpBuilder bEntry(currentEntryBlock, currentEntryBlock->begin());
  Value v = bEntry.create<InlineAsmOp>(unknownLoc, type, ValueRange({}), asm_string, constraints, has_side_effects, is_align_stack, asm_dialect).getResult(0);
...
Value Importer::processValue(llvm::Value *value) {
  ...
  if (auto *i = dyn_cast<llvm::InlineAsm>(value))
    return processInlineAsm(i);
...
LogicalResult Importer::processInstruction(llvm::Instruction *inst) {
  ...
  case llvm::Instruction::ICmp: {
    ...
    if (!lhs || !rhs)
      return failure();
    if (lhs.getType().dyn_cast<LLVMPointerType>()) {
      lhs = b.create<PtrToIntOp>(loc, b.getI64Type(), lhs);
    }
    if (rhs.getType().dyn_cast<LLVMPointerType>()) {
      rhs = b.create<PtrToIntOp>(loc, b.getI64Type(), rhs);
    }

    auto pred = getICmpPredicate(cast<llvm::ICmpInst>(inst)->getPredicate());
    auto predAttr = mlir::LLVM::ICmpPredicateAttr::get(context, pred);
    if (lhs.getType().dyn_cast<mlir::VectorType>()) {
      mlir::VectorType type = mlir::VectorType::get(lhs.getType().dyn_cast<mlir::VectorType>().getShape(), b.getI1Type());
      v = b.create<ICmpOp>(loc, type, predAttr, lhs, rhs);
    } else {
      v = b.create<ICmpOp>(loc, pred, lhs, rhs);
    }
  ...
  case llvm::Instruction::Select: {
    llvm::SelectInst *select = cast<llvm::SelectInst>(inst);
    Value condition = processValue(select->getCondition());
    Value trueValue = processValue(select->getTrueValue());
    Value falseValue = processValue(select->getFalseValue());

    v = b.create<LLVM::SelectOp>(loc, condition, trueValue, falseValue);

    return success();
  }

  case llvm::Instruction::ShuffleVector: {
    llvm::ShuffleVectorInst* shuffle = cast<llvm::ShuffleVectorInst>(inst);

    llvm::SmallVector<int, 4> mask;
    shuffle->getShuffleMask(mask);

    std::vector<Attribute> mask_mlir;
    for (auto i: mask) {
      mask_mlir.push_back(b.getI32IntegerAttr(i));
    }

    v = b.create<mlir::LLVM::ShuffleVectorOp>(loc, processValue(shuffle->getOperand(0)), processValue(shuffle->getOperand(1)), b.getArrayAttr(mask_mlir));

    return success();
  }

  case llvm::Instruction::InsertElement: {
    llvm::InsertElementInst* insert = cast<llvm::InsertElementInst>(inst);
    v = b.create<mlir::LLVM::InsertElementOp>(loc, processType(insert->getType()), processValue(insert->getOperand(0)), processValue(insert->getOperand(1)), processValue(insert->getOperand(2)));
    return success();
  }

  case llvm::Instruction::ExtractElement: {
    llvm::ExtractElementInst* extract = cast<llvm::ExtractElementInst>(inst);
    v = b.create<mlir::LLVM::ExtractElementOp>(loc, processType(extract->getType()), processValue(extract->getOperand(0)), processValue(extract->getOperand(1)));
    return success();
  }

  case llvm::Instruction::AtomicCmpXchg: {
    llvm::AtomicCmpXchgInst* acx = cast<llvm::AtomicCmpXchgInst>(inst);
    Type type = processType(acx->getType());
    Value ptr = processValue(acx->getOperand(0));
    Value cmp = processValue(acx->getOperand(1));
    Value val = processValue(acx->getOperand(2));
    AtomicOrdering success_ordering = getLLVMAtomicOrdering(acx->getSuccessOrdering());
    AtomicOrdering failure_ordering = getLLVMAtomicOrdering(acx->getFailureOrdering());
    v = b.create<mlir::LLVM::AtomicCmpXchgOp>(loc, type, ptr, cmp, val, success_ordering, failure_ordering);

    return success();
  }

  case llvm::Instruction::AtomicRMW: {
    llvm::AtomicRMWInst* rmw = cast<llvm::AtomicRMWInst>(inst);
    Type type = processType(rmw->getType());
    AtomicBinOp bin_op = (mlir::LLVM::AtomicBinOp)rmw->getOperation();
    Value ptr = processValue(rmw->getPointerOperand());
    Value val = processValue(rmw->getValOperand());
    AtomicOrdering ordering = getLLVMAtomicOrdering(rmw->getOrdering());

    v = b.create<AtomicRMWOp>(loc, type, bin_op, ptr, val, ordering);

    return success();
  }

  case llvm::Instruction::ExtractValue: {
    llvm::ExtractValueInst* ev = cast<llvm::ExtractValueInst>(inst);
    Type type = processType(ev->getType());
    Value container = processValue(ev->getAggregateOperand());

    std::vector<Attribute> attributes;
    for (auto i: ev->getIndices()) {
      attributes.push_back(b.getI64IntegerAttr(i));
    }
    ArrayAttr position = b.getArrayAttr(attributes);

    v = b.create<ExtractValueOp>(loc, type, container, position);

    return success();
  }

  case llvm::Instruction::FNeg: {
    llvm::UnaryOperator* fneg = cast<llvm::UnaryOperator>(inst);
    Type type = processType(fneg->getType());
    Value operand = processValue(fneg->getOperand(0));

    v = b.create<FNegOp>(loc, type, operand);
    return success();
  }
  ...
  case llvm::Instruction::FCmp: {
    ...
    auto pred = getFCmpPredicate(cast<llvm::FCmpInst>(inst)->getPredicate());
    auto predAttr = mlir::LLVM::FCmpPredicateAttr::get(context, pred);
    if (lhs.getType().dyn_cast<mlir::VectorType>()) {
      mlir::VectorType type = mlir::VectorType::get(lhs.getType().dyn_cast<mlir::VectorType>().getShape(), b.getI1Type());
      v = b.create<FCmpOp>(loc, type, predAttr, lhs, rhs, LLVM::FMFAttr({}));
    } else {
      v = b.create<FCmpOp>(loc, b.getI1Type(), pred, lhs, rhs, LLVM::FastmathFlags());
    }
  ...
  case llvm::Instruction::Switch: {
    llvm::SwitchInst* sw = cast<llvm::SwitchInst>(inst);
    ...
    std::vector<ValueRange> caseOperands;

    auto phiToBranchArgs = [this](llvm::Instruction* inst, llvm::BasicBlock* bb, SmallVectorImpl<Value>* blockArguments) {
      for (auto phi = bb->begin(); isa<llvm::PHINode>(phi); ++phi) {
        auto *PN = cast<llvm::PHINode>(&*phi);
        Value value = processValue(PN->getIncomingValueForBlock(inst->getParent()));
    ...
    condition = processValue(sw->getCondition());
    if (!condition.getType().isSignlessInteger(32)) {
      condition = b.create<LLVM::BitcastOp>(loc, b.getI32Type(), condition);
    }

    {
      OpBuilder::InsertionGuard guard(b);

      if (blocks[sw->getDefaultDest()]) {
        defaultDest = blocks[sw->getDefaultDest()];
      } else {
        defaultDest = b.createBlock(b.getBlock()->getParent());
        b.setInsertionPointToStart(defaultDest);

        if (failed(processBasicBlock(sw->getDefaultDest(), defaultDest))) {
          return failure();
        }
      }

      for (auto phi = sw->getDefaultDest()->begin(); isa<llvm::PHINode>(phi); ++phi) {
        auto *PN = cast<llvm::PHINode>(&*phi);
        Value value = processValue(PN->getIncomingValueForBlock(sw->getParent()));
        if (!value) return failure();
        defaultOperands.push_back(value);
      }

      // if (failed(processBranchArgs(sw, sw->getDefaultDest(), defaultOperands))) {
      //   return failure();
      // }
    }


    for (auto c = sw->case_begin(); c != sw->case_end(); c++) {
      Block* bb;
      OpBuilder::InsertionGuard guard(b);

      if (blocks[c->getCaseSuccessor()]) {
        bb = blocks[c->getCaseSuccessor()];
      } else {
        bb = b.createBlock(b.getBlock()->getParent());
        b.setInsertionPointToStart(bb);

        if (failed(processBasicBlock(c->getCaseSuccessor(), bb))) {
          return failure();
        }
      }

      llvm::SmallVector<Value, 4> caseOperand;
      for (auto phi = c->getCaseSuccessor()->begin(); isa<llvm::PHINode>(phi); ++phi) {
        auto *PN = cast<llvm::PHINode>(&*phi);
        Value value = processValue(PN->getIncomingValueForBlock(sw->getParent()));
        if (!value) return failure();
        caseOperand.push_back(value);
      }

      // FIXME: this cause bus error?
      // if (failed(processBranchArgs(sw, c->getCaseSuccessor(), caseOperand))) {
      //   return failure();
      // }

      caseValues.push_back(*c->getCaseValue()->getValue().getRawData());
      caseDests.push_back(bb);
      caseOperands.push_back(caseOperand);
    }

    b.create<SwitchOp>(loc, std::move(condition), std::move(defaultDest), std::move(defaultOperands), std::move(caseValues), std::move(caseDests), std::move(caseOperands));
