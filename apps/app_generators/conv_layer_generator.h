...
#ifndef CONV_LAYER_GENERATOR_H_
#define CONV_LAYER_GENERATOR_H_
...
template <typename T>
void GenerateConvLayer(int IC, int H, int W, T* IF, int OC, int KSIZE, T* Kernel, T Bias) {
  Var c, x, y;

  Buffer<T> kernel{Kernel, KSIZE, KSIZE, IC, OC};
  Buffer<T> input_feature_map{IF, W, H, IC};

  Func conv;
  // ordered reduction domain CHW => r.x = W, r.y = H, r.z = C
  RDom r(0, KSIZE, 0, KSIZE, 0, IC);

  conv(y, x, c) = Expr(Bias);
  conv(y, x, c) += kernel(r.x, r.y, r.z, c) * input_feature_map(y + r.x, x + r.y, r.z);

  conv.bound(c, 0, OC);
  conv.bound(x, 0, H - KSIZE + 1);
  conv.bound(y, 0, W - KSIZE + 1);

  conv.compile_to_lowered_stmt("conv_.stmt", {}, Halide::Text);
  return;
}

template <typename T>
void GenerateConvLayer(int IC, int H, int W, int OC, int KSIZE) {
  Var c, x, y;

  ImageParam input_feature_map{type_of<T>(), 3};
  ImageParam kernel{type_of<T>(), 4};
  ImageParam bias{type_of<T>(), 1};
  ...
  input_feature_map.dim(0).set_bounds(0, W);
  input_feature_map.dim(1).set_bounds(0, H);
  input_feature_map.dim(2).set_bounds(0, IC);
  kernel.dim(0).set_bounds(0, KSIZE);
  kernel.dim(1).set_bounds(0, KSIZE);
  kernel.dim(2).set_bounds(0, IC);
  kernel.dim(3).set_bounds(0, OC);
  bias.dim(0).set_bounds(0, 1);

  conv.compile_to_lowered_stmt("conv.stmt", {input_feature_map, kernel, bias}, Halide::Text);
